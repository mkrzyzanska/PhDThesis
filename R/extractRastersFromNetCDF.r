...
if(crop==TRUE){
    # First create a separate directory for the cropped layer
    dir.create(paste(path2layers,"\\cropped\\",variable,sep="")) 
     # Save the cropped layer
writeRaster(cropped_env[[1:length(names(r))]],filename=paste(path2layers,"\\cropped\\",variable,"\\",filenames[1:length(names(r))],".tiff",sep=""),format="GTiff", overwrite=TRUE,bylayer=TRUE)
    }
if(mask==TRUE){
    # Create the directory
    dir.create(paste(path2layers,"\\masked\\",variable,sep=""))
    # Mask past layers
    masked_env<-mask(cropped_env, area)
    #Write masked layers to the file  
    writeRaster(masked_env[[1:length(names(r))]],filename=paste(path2layers,"\\masked\\",variable,"\\",filenames[1:length(names(r))],".tiff",sep=""),format="GTiff",overwrite=TRUE,bylayer=TRUE)
        }
if(!is.null(path2present)){   
    return(c(brick(cropped_pr,cropped_env),brick(masked_pr,masked_env)))
    }else{return(c(cropped_env,masked_env))}
}